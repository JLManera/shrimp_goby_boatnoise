...
Manera JL, Martin JM, Palacios MM, Mason RT, McCormick MI, Wong BBM. 2025. Data from: Boat noise alters behaviour in a fish-shrimp mutualism. DOI: 10.17605/OSF.IO/3X5YF. (to be updated upon publication)
...
wide_data <- workingdf5 |>
  pivot_wider(
    id_cols = c(burrow_species, burrow_species_stage, burrow, stage, species, boat_id, stroke, site, number_conspecifics, shrimp_species, cum_sum_boat_trials_per_site, date),
...
final_df <- inner_join(goby_df, shrimp_df, by = "burrow_stage") |> 
  ...
  filter(site != "SITE O") |> 
  mutate(parsed_date = mdy(paste(date, "2023")),
         experiment_day = dense_rank(parsed_date))
```
...
final_df <- inner_join(goby_df, shrimp_df, by = "burrow_stage") |> 
  mutate(
    shrimp_contact = ifelse(is.na(shrimp_contact),
                            ifelse(goby_out == 1, 0, 1 - goby_out),
                            shrimp_contact)) |> 
  filter(site != "SITE O") |> 
  mutate(
    parsed_date    = mdy(paste(date, "2023")),
    experiment_day = as.integer(parsed_date - min(parsed_date, na.rm = TRUE)) + 1
  )
...
```{r}
get_prior(zoib_formula, data = final_df)
```
...
ggplot(df, aes(x)) +
  geom_line(aes(y = Logistic, color = "Logistic(0, 3)")) + 
  geom_line(aes(y = Student_t, color = "Student_t(3, 0, 2.5)")) +
  geom_line(aes(y = Normal, color = "Normal(0, 3)")) +
...
```{r}
goby_out_plot3
ggsave("./2-figures/goby_out_plot.png", plot = goby_out_plot3, units = "px", width = 2200, height = 1400, dpi = 300)
```
...
shrimp_out_plot3
ggsave("./2-figures/shrimp_out_plot.png", plot = shrimp_out_plot3, units = "px", width = 2200, height = 1400, dpi = 300)
```
...
overall_mean <- shrimp_contact_model |>
  emmeans(~ 1, epred = TRUE, re_formla = NULL, rg.limit = 14000)
...
ggsave("./2-figures/shrimp_contact_plot.png", plot = shrimp_contact_plot3, units = "px", width = 2200, height = 1400, dpi = 300)
```
...
write_csv(model_estimates, "./2-figures/supplementary_tables/Table_S4.csv")
```
...
write_csv(goby_out_contrasts_table, "./2-figures/supplementary_tables/Table_S5.csv")
...
write_csv(shrimp_out_contrasts_table, "./2-figures/supplementary_tables/Table_S6.csv")
...
write_csv(shrimp_contact_contrasts_table, "./2-figures/supplementary_tables/Table_S7.csv")
```
...
write_csv(table_S1, "./2-figures/supplementary_tables/Table_S1.csv")
```

# Supplementary analysis 

Although sites in the experimental design were spatially distinct, boats were tested multiple times at the same sites across the experiment. This raises the possibility that some burrows may have potentially experienced prior boat-noise exposure before their focal trial. Such cumulative exposure could influence the behavioural responses we observed. To assess this, we conducted a supplementary analysis.

We quantified potential cumulative prior exposure at each site using the variable cum_sum_boat_trials_per_site, defined as the cumulative number of boat-noise treatments that had occurred at a site before the current trial. This variable was added as a predictor to the original models. We then compared models with and without the predictor using LOOIC, examined the estimated trend of each behavioural outcome in relation to cumulative exposure, and visually and quantitatively compared effect sizes. Finally, we assessed whether inclusion of the cumulative exposure variable altered the inference of the main effects.

```{r}
ggplot(data = final_df, aes(x = cum_sum_boat_trials_per_site, y = goby_out)) +
  geom_smooth(method = "lm") +
  geom_jitter()

ggplot(data = final_df, aes(x = cum_sum_boat_trials_per_site, y = shrimp_out)) +
  geom_smooth(method = "lm") +
  geom_jitter()

ggplot(data = final_df, aes(x = cum_sum_boat_trials_per_site, y = shrimp_contact)) +
  geom_smooth(method = "lm") +
  geom_jitter()
```
...
Here we run the models with 'cum_sum_boat_trials_per_site' which is the cumulative sum of the number of boat trials at each site. This is to see if potential prior exposure to boat noise has a an effect on behavior

```{r}
cumulative_sum_goby_out_formula <- bf(
  goby_out ~ stroke*stage + goby_number + shrimp_number + shrimp_species + cum_sum_boat_trials_per_site + (1 | site/burrow),
  phi ~ stroke*stage + goby_number + shrimp_number + shrimp_species + cum_sum_boat_trials_per_site + (1 | site/burrow),
  zoi ~ stroke*stage + goby_number + shrimp_number + shrimp_species + (1 | site/burrow),
  coi ~ stroke*stage + goby_number + shrimp_number + shrimp_species + (1 | site/burrow),
  family = zero_one_inflated_beta()
  )

cumulative_sum_goby_out_model <- brm(
  cumulative_sum_goby_out_formula,
  data = final_df,
  family = zero_one_inflated_beta(),
  prior = priors,
  control = list(adapt_delta = 0.99,
  ...
  backend = "cmdstanr",
  seed = 1,
  file = "./3-models/cumulative_sum_goby_out_model"
)
```
...
```{r}
summary(cumulative_sum_goby_out_model)

plot(cumulative_sum_goby_out_model, ask = F)

pp_check(cumulative_sum_goby_out_model)
```

### Model predictive performance

```{r}
goby_out_model <- add_criterion(goby_out_model, criterion = "loo")
cumulative_sum_goby_out_model <- add_criterion(cumulative_sum_goby_out_model, criterion = "loo")

goby_out_cumulative_loo_compare <- as.data.frame(loo_compare(goby_out_model, cumulative_sum_goby_out_model))
goby_out_cumulative_loo_compare
```

the addition of 'cum_sum_boat_trials_per_site' does not improve model predictive performance

### Effects of previous boat trials

```{r}
cum_sum_boat_trials_per_site_effect_goby_out <- emtrends(cumulative_sum_goby_out_model, ~ 1, var = "cum_sum_boat_trials_per_site", epred = TRUE, re_formula = NA)
cum_sum_boat_trials_per_site_effect_goby_out
```

'cum_sum_boat_trials_per_site' does not have a meaningful effect on the proportion of time gobies spent out of the burrow 

### Comparison of effect estimates

```{r}
cumulative_sum_goby_out_model_contrast_effect <- cumulative_sum_goby_out_model |>
  emmeans(~ stroke*stage, 
          epred = TRUE,
          re_formla = NULL,
          rg.limit =14000) |> 
...
cumulative_sum_goby_out_results.table <- as.data.frame(cumulative_sum_goby_out_model_contrast_effect) |> 
  dplyr::filter(contrast %in% c("control Pre - 2 Pre", "control Pre - 4 Pre","4 Pre - 2 Pre", "control Pre - control During", "control Pre - control Post", "4 Pre - 4 During", "4 Pre - 4 Post", "2 Pre - 2 During", "2 Pre - 2 Post"))

cumulative_sum_goby_out_results.table
goby_out_results.table

cumulative_sum_goby_out_results.table$dataset <- "Cumulative sum model"
goby_out_results.table$dataset <- "Non-cumulative model"

goby_contrast_plot_data <- bind_rows(
  cumulative_sum_goby_out_results.table,
  goby_out_results.table
)

ggplot(goby_contrast_plot_data,
       aes(x = estimate, y = contrast, color = dataset)) +
  ...
  theme_classic() +
  labs(
    x = "Posterior median (with 95% HPD interval)",
    y = "Contrasts",
    title = "Comparison of contrasts between cumulative-sum and original models"
  )
```
...
```{r}
cumulative_sum_shrimp_out_formula <- bf(
  shrimp_out ~ stroke*stage + goby_number + shrimp_number + shrimp_species + cum_sum_boat_trials_per_site + (1 | site/burrow),
  phi ~ stroke*stage + goby_number + shrimp_number + shrimp_species + cum_sum_boat_trials_per_site + (1 | site/burrow),
  zoi ~ stroke*stage + goby_number + shrimp_number + shrimp_species + (1 | site/burrow),
  coi ~ stroke*stage + goby_number + shrimp_number + shrimp_species + (1 | site/burrow),
  family = zero_one_inflated_beta()
...
cumulative_sum_shrimp_out_model <- brm(
  cumulative_sum_shrimp_out_formula,
  data = final_df,
  family = zero_one_inflated_beta(),
  prior = priors,
  control = list(adapt_delta = 0.99,
  ...
  backend = "cmdstanr",
  seed = 1,
  file = "./3-models/cumulative_sum_shrimp_out_model"
)
```
...
```{r}
summary(cumulative_sum_shrimp_out_model)

plot(cumulative_sum_shrimp_out_model, ask = F)

pp_check(cumulative_sum_shrimp_out_model)
```

### Model predictive performance

```{r}
shrimp_out_model <- add_criterion(shrimp_out_model, criterion = "loo")
cumulative_sum_shrimp_out_model <- add_criterion(cumulative_sum_shrimp_out_model, criterion = "loo")

shrimp_out_cumulative_loo_compare <- as.data.frame(loo_compare(shrimp_out_model, cumulative_sum_shrimp_out_model))
shrimp_out_cumulative_loo_compare
```
...
```{r}
cum_sum_boat_trials_per_site_effect_shrimp_out <- emtrends(cumulative_sum_shrimp_out_model, ~ 1, var = "cum_sum_boat_trials_per_site", epred = TRUE, re_formula = NA)
cum_sum_boat_trials_per_site_effect_shrimp_out
```

'cum_sum_boat_trials_per_site' does not have a meaningful effect on the proportion of time shrimp spent out of the burrow 

### Comparison of effect estimates

```{r}
cumulative_sum_shrimp_out_model_contrast_effect <- cumulative_sum_shrimp_out_model |>
  emmeans(~ stroke*stage, 
          epred = TRUE,
          re_formla = NULL,
          rg.limit =14000) |> 
...
cumulative_sum_shrimp_out_results.table <- as.data.frame(cumulative_sum_shrimp_out_model_contrast_effect) |> 
  dplyr::filter(contrast %in% c("control Pre - 2 Pre", "control Pre - 4 Pre","4 Pre - 2 Pre", "control Pre - control During", "control Pre - control Post", "4 Pre - 4 During", "4 Pre - 4 Post", "2 Pre - 2 During", "2 Pre - 2 Post"))

cumulative_sum_shrimp_out_results.table
shrimp_out_contrasts

cumulative_sum_shrimp_out_results.table$dataset <- "Cumulative sum model"
shrimp_out_contrasts$dataset <- "Non-cumulative model"

shrimp_contrast_plot_data <- bind_rows(
  cumulative_sum_shrimp_out_results.table,
  shrimp_out_contrasts
)

ggplot(shrimp_contrast_plot_data,
       aes(x = estimate, y = contrast, color = dataset)) +
  geom_point(position = position_dodge(width = 0.6), size = 2) +
  geom_errorbarh(aes(xmin = lower.HPD, xmax = upper.HPD),
                 position = position_dodge(width = 0.6), height = 0.2) +
...
```{r}
cumulative_sum_shrimp_contact_formula <- bf(
  shrimp_contact ~ stroke*stage + goby_number + shrimp_number + shrimp_species + cum_sum_boat_trials_per_site + (1 | site/burrow),
  phi ~ stroke*stage + goby_number + shrimp_number + shrimp_species + cum_sum_boat_trials_per_site + (1 | site/burrow),
  zoi ~ stroke*stage + goby_number + shrimp_number + shrimp_species + (1 | site/burrow),
  coi ~ stroke*stage + goby_number + shrimp_number + shrimp_species + (1 | site/burrow),
  family = zero_one_inflated_beta()
...
cumulative_sum_shrimp_contact_model <- brm(
  cumulative_sum_shrimp_contact_formula,
  data = final_df,
  family = zero_one_inflated_beta(),
  prior = priors,
  control = list(adapt_delta = 0.99,
  ...
  backend = "cmdstanr",
  seed = 1,
  file = "./3-models/cumulative_sum_shrimp_contact_model"
)
```
...
```{r}
summary(cumulative_sum_shrimp_contact_model)

plot(cumulative_sum_shrimp_contact_model, ask = F)

pp_check(cumulative_sum_shrimp_contact_model)
```

### Model predictive performance

```{r}
shrimp_contact_model <- add_criterion(shrimp_contact_model, criterion = "loo")
cumulative_sum_shrimp_contact_model <- add_criterion(cumulative_sum_shrimp_contact_model, criterion = "loo")

shrimp_contact_cumulative_loo_compare <- as.data.frame(loo_compare(shrimp_contact_model, cumulative_sum_shrimp_contact_model))
shrimp_contact_cumulative_loo_compare
```
...
```{r}
cum_sum_boat_trials_per_site_effect_shrimp_contact <- emtrends(cumulative_sum_shrimp_contact_model, ~ 1, var = "cum_sum_boat_trials_per_site", epred = TRUE, re_formula = NA)
cum_sum_boat_trials_per_site_effect_shrimp_contact
```

'cum_sum_boat_trials_per_site' does not have a meaningful effect on the proportion of time shrimp spent in contact with a goby

### Comparison of effect estimates

```{r}
cumulative_sum_shrimp_contact_model_contrast_effect <- cumulative_sum_shrimp_contact_model |>
  emmeans(~ stroke*stage, 
          epred = TRUE,
          re_formla = NULL,
          rg.limit =14000) |> 
...
cumulative_sum_shrimp_contact_results.table <- as.data.frame(cumulative_sum_shrimp_contact_model_contrast_effect) |> 
  dplyr::filter(contrast %in% c("control Pre - 2 Pre", "control Pre - 4 Pre","4 Pre - 2 Pre", "control Pre - control During", "control Pre - control Post", "4 Pre - 4 During", "4 Pre - 4 Post", "2 Pre - 2 During", "2 Pre - 2 Post"))

cumulative_sum_shrimp_contact_results.table
shrimp_contact_contrasts

cumulative_sum_shrimp_contact_results.table$dataset <- "Cumulative sum model"
shrimp_contact_contrasts$dataset <- "Non-cumulative model"

shrimp_contact_contrast_plot_data <- bind_rows(
  cumulative_sum_shrimp_contact_results.table,
  shrimp_contact_contrasts
)

ggplot(shrimp_contact_contrast_plot_data,
       aes(x = estimate, y = contrast, color = dataset)) +
  geom_point(position = position_dodge(width = 0.6), size = 2) +
  geom_errorbarh(aes(xmin = lower.HPD, xmax = upper.HPD),
                 position = position_dodge(width = 0.6), height = 0.2) +
...
```{r}
short_label <- function(x) dplyr::recode(
  x,
  "Proportion of time gobies spent outside of the burrow"  = "Goby: time outside burrow",
  "Proportion of time shrimp spent outside of the burrow"  = "Shrimp: time outside burrow",
  "Proportion of time shrimp spent in contact with a goby" = "Shrimp–goby contact",
  .default = x
)

cumulative_loo_compare_clean <-
  bind_rows(
    goby_out_cumulative_loo_compare  |> rownames_to_column("Model") |>
      mutate(Behavioural_measure = "Proportion of time gobies spent outside of the burrow"),
    shrimp_out_cumulative_loo_compare |> rownames_to_column("Model") |>
      mutate(Behavioural_measure = "Proportion of time shrimp spent outside of the burrow"),
    shrimp_contact_cumulative_loo_compare |> rownames_to_column("Model") |>
      mutate(Behavioural_measure = "Proportion of time shrimp spent in contact with a goby")
  ) |>
  mutate(
    Behavioural_measure = short_label(Behavioural_measure),
    Model = if_else(str_detect(Model, "^cumulative_sum"), "Cumulative-sum model", "Non-cumulative model")
  ) |>
  select(
    Behavioural_measure, Model,
    elpd_loo, se_elpd_loo, looic, elpd_diff, se_diff
  ) |>
  rename(
    `Behavioural measure` = Behavioural_measure,
    `SE (elpd_loo)` = se_elpd_loo,
    `SE (elpd_diff)` = se_diff
  )

write_csv(cumulative_loo_compare_clean, "./2-figures/supplementary_tables/Table_S2.csv")

cumulative_effects_clean <-
  bind_rows(
    as.data.frame(cum_sum_boat_trials_per_site_effect_goby_out) |>
      mutate(Behavioural_measure = "Proportion of time gobies spent outside of the burrow"),
    as.data.frame(cum_sum_boat_trials_per_site_effect_shrimp_out) |>
      mutate(Behavioural_measure = "Proportion of time shrimp spent outside of the burrow"),
    as.data.frame(cum_sum_boat_trials_per_site_effect_shrimp_contact) |>
      mutate(Behavioural_measure = "Proportion of time shrimp spent in contact with a goby")
  ) |>
  mutate(Behavioural_measure = short_label(Behavioural_measure)) |>
  select(any_of(c("Behavioural_measure", "cum_sum_boat_trials_per_site.trend", "lower.HPD", "upper.HPD"))) |>
  rename(
    `Behavioural measure` = Behavioural_measure,
    `Trend per additional prior boat trial` = cum_sum_boat_trials_per_site.trend,
    `Lower 95% CrI` = lower.HPD,
    `Upper 95% CrI` = upper.HPD
  )
cumulative_effects_clean
write_csv(cumulative_effects_clean, "./2-figures/supplementary_tables/Table_S3.csv")
```