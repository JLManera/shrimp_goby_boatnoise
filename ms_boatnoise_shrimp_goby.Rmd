...
**SUMMARY**  
Human activities are reshaping marine soundscapes, with boat noise a near-constant source of underwater noise and a primary contributor to rising oceanic noise pollution. Despite increasing concern, empirical data for most marine life, particularly in natural settings, remains sparse. Fish and crustaceans are underrepresented in noise pollution research, despite their auditory ranges make them especially susceptible to low-frequency typical of boat engines. Accordingly, this study employs a field-based experimental approach to investigate the impacts of boat noise on an iconic behavioural mutualism between gobies (Amblyeleotris steinitzi) and shrimp (Alpheus spp). Our findings demonstrate that both species exhibit increased burrow use in response to boat noise, suggesting a heightened perception of risk. Despite the perceived risk triggered by boat noise, the behaviour of the two species remained tightly correlated and communication—measured as antennae contact—remained unchanged. Notably, shrimp exhibited stronger responses to 2-stroke engines, whereas gobies responded more strongly to 4-stroke engines, indicating species-specific sensitivities to different noise spectra. These results highlight the complex and taxon-specific effects of noise pollution on marine species and emphasize the need for targeted research to inform conservation strategies aimed at mitigating anthropogenic acoustic disturbances in marine ecosystems .
...
Here we are dealing with miss focal species during specific segments of the trial.   

We make the assumption that when a **goby is present in other videos** (i.e. we know it exists) but **missing from a given trial segment** it is **'away'** and out of frame for the entire trial. In the case of shrimps, we assume it **inside the burrow the entire time**. These assumptions are based off our teams observation of these species, and the previously reported biological tendencies of each species. Because of this assumption we are assigning the respective value to the behviour in cases where one of the species is known to be present in a burrow but not viewed for the length of the trial.   

🚧 Jack, maybe you could break this up and add a little more detail, I got a little lost in the code 🚧 
...
Restructure the data frame for easier analysis. We also write this file as **💿 final_df.csv** in the the 📁 `./2-data` folder. This is what we will use for analysis.  
...
🚧 Jack, you might consider wiring the brms models to a folder called '4-mods' 🚧   
...
```{r}
goby_out_model <-  readRDS(file = "goby_out_model.rds")
```
...
🚧 Interesting that a 16% of the observations are not being predicted very well by the model, do we think this is an issue 🚧 

Using leave one out (loo) measure of fit, the model appears to preform ok, 16% Pareto k estimates falls outside the good range (0.7, 1]   
...
ggsave("./3-figs/goby_out_plot3.pdf", plot = goby_out_plot3, units = "px", width = 2200, height = 1400)
```

### Effects of covariates on the proportion of time gobies spent outside of the burrow
...
```{r}
shrimp_out_model <-  readRDS(file = "shrimp_out_model.rds")
```
...
ggsave("./3-figs/shrimp_out_plot3.pdf", plot = shrimp_out_plot3, units = "px", width = 2200, height = 1400)
```

### Effects of covariates on the proportion of time shrimp spent outside of the burrow
...
ggsave("./3-figs/shrimp_contact_plot3.pdf", plot = shrimp_contact_plot3, units = "px", width = 2200, height = 1400)
```

### Modelling the effect of covariates on the proportion of time spent in contact
